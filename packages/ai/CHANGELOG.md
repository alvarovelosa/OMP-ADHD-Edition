...
- Added `/models` (alias for `/v1/models`) and `/chat/completions` (alias for `/v1/chat/completions`) route aliases to `auth-gateway` HTTP server.
...
- Restored Ollama-compatible `/api/tags` and `/api/chat` routes in `auth-gateway` alongside OpenAI/Hammer route aliases.
